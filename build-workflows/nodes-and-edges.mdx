...
| --- | --- | --- |
| `answer` | Message nodes | `label`, `index`, `editable`, `removable`. Canvas: `parentId`, `draggable: false`, `extent: "parent"` (send `autoLayout: true` to snap `{ x: 138, y }`). Optional `buttonPayload`, `carouselCardIndex`. |
| `conditionEvaluation` | `if` | `label` / execution `title` must be `true` or `false`. Same canvas fields as answers (`draggable: false`, `extent: "parent"`). |
| `switchCaseEvaluation` | `switch` | `label`, `conditions`, `evaluationMode`. Same canvas fields. |
| `noSelectionFallback` | Message or `switch` | **Never** edge back to the parent. Mode A: `shouldSendReplyMessage` + `unknownAnswerReplyMessage`, empty `connections`. Mode B: `connections.out` to a different node (human / AI / pointer). |
